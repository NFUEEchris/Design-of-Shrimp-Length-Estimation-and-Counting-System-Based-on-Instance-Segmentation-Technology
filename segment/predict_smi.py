...
def run(
    ...
    cfg = get_config()
    cfg.merge_from_file(r"C:\code\YOLOv7-Pytorch-Segmentation-master\tracker\track._smi.yaml")
    ...
    # bytetrck=byte_tracker.BYTETracker()
    ...
    id_dict=dict()
    for path, im, im0s, vid_cap, s in dataset: #per pic
        with dt[0]:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            annotator = Annotator(im0, line_width=line_thickness, font_size=2, example=str(names))
            ...
            if det is not None and len(det):
                ...
                for mask in masks_np:
                    ...
                    all_contour_length.append(contour_length/2)
                ...
                # confs=reversed(det[:, :6])[:,4]
                # clss=reversed(det[:, :6])[:,5]    
                # outputs = deepsort.update(xywhs.cpu(), confs.cpu(), clss.cpu(), im0)
                #============DEEPSORT========================
                boxes = scale_coords(im.shape[2:], det[:, :4], im0.shape)
                boxes = boxes.cpu().numpy()
                detections = det.cpu().numpy()
                detections[:, :4] = boxes
                
                online_targets = tracker.update(detections, im0) #to list
                print("online_targets:",online_targets)
                #====================================================TRACKER===============================================
                #if len(outputs) > 0:
                if len(online_targets) > 0:

                    for (*xyxy, conf, cls)  in zip(reversed(det[:, :6])): #per bbox ,output ,outputs
                        
                        #============DEEPSORT========================
                        # id = output[4]
                        # if str(id) not in id_dict:
                        #     id_dict[str(id)]=0
                        #============DEEPSORT========================
                        ...
                        #=======================================arduino==============================================================
                        if  dis_A<push_dis and line_status_A=="right"  and all_contour_length[length_count]<100 : #and id_dict.get(str(id))==0
                            motor="motor_A"                       
                            print("motor_A")
                            motor_fun(1)
                            # id_dict[str(id)]=id_dict.get(str(id))+1

                        elif dis_B<push_dis and line_status_B=="right" and all_contour_length[length_count]<200 and all_contour_length[length_count]>100 : #and id_dict.get(str(id))==0
                            print("motor_B")
                            motor="motor_B"
                        ...
                        elif dis_C<push_dis and line_status_C=="right" and all_contour_length[length_count]>200 : #and id_dict.get(str(id))==0
                            print("motor_C")
                            motor="motor_C"
                            motor_fun(3)
                            # id_dict[str(id)]=id_dict.get(str(id))+1
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            ...
                            label = None if hide_labels else (names[c] if hide_conf else f'pixel: {all_contour_length[length_count]:.2f} status:{motor}') #-{id}
                    ...
                    for *xyxy, conf, cls in reversed(det[:, :6]):
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            c = int(cls)  # integer class
                            label = None if hide_labels else (names[c] if hide_conf else f'pixel: {all_contour_length[length_count]:.2f}')
                            annotator.box_label(xyxy, label, color=colors(c, True))
                        if save_crop:
                            save_one_box(xyxy, imc, file=save_dir / 'crops' / names[c] / f'{p.stem}.jpg', BGR=True)
                        length_count+=1
